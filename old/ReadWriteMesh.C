...
# include <sys/types.h>
#endif // if defined(HAVE_SYS_TYPES_H)

#if defined(HAVE_SYS_TIME_H)
# include <sys/time.h>
#endif // if defined(HAVE_SYS_TIME_H)

#if defined(HAVE_SYS_TIMES_H)
# include <sys/times.h>
#endif // if defined(HAVE_SYS_TIMES_H)
...
SurfaceMeshOld * SurfaceMeshOld::readPoly(std::string input_name)
{
    ...
    for (auto curr : surfmesh->vertices())
    {
        curr.x   = tetio.pointlist[n * 3];
        curr.y   = tetio.pointlist[n * 3 + 1];
        curr.z   = tetio.pointlist[n * 3 + 2];
        curr.sel = true;

        if (tetio.pointmarkerlist != NULL)
        {
            curr.m = tetio.pointmarkerlist[n];
        }
        ++n;
    }
    ...
    /*
       ...
       for (n = 0; n < tetio.numberofpoints; n++) {
        FLTVECT vertex;
        vertex.x   = tetio.pointlist[n * 3];
        vertex.y   = tetio.pointlist[n * 3 + 1];
        vertex.z   = tetio.pointlist[n * 3 + 2];
        vertex.sel = true;

        if (tetio.pointmarkerlist != NULL) {
            vertex.m = tetio.pointmarkerlist[n];
        }
        surfmesh->vertex[n] = vertex;
       }
     */
    ...
    for (n = 0; n < tetio.numberoffacets; n++)
    {
        ...
        surfmesh->face[n].a = tetio.facetlist[n].polygonlist[0].vertexlist[0] -
                              1;
        surfmesh->face[n].b = tetio.facetlist[n].polygonlist[0].vertexlist[1] -
                              1;
        surfmesh->face[n].c = tetio.facetlist[n].polygonlist[0].vertexlist[2] -
                              1;
...
SurfaceMeshOld * SurfaceMeshOld::readOFF(std::string input_name)
{
    ...
    else if (m == 3)
    {
        ...
        if (character != '\n')
        {
            ...
            {
                printf(
                    "Read error. Expected 4 floats for the RGBA values of the first face.\n");
                return NULL;
            }
    ...
    else
    {
        ...
        // We have already read the vertex coordinates
...
void GemMesh_writeDolfin(GemMesh *out, char *filename)
{
    ...
    for (i = 0; i < out->num_cells; i++)
    {
        ...
        fprintf(fout,
                "      <tetrahedron index=\"%d\" v0=\"%d\" v1=\"%d\" v2=\"%d\" " \
                "v3=\"%d\" />\n",
                i,
                conn[indices[i * 4]],
                conn[indices[i * 4 + 1]], \
    ...
    for (i = 0; i < out->num_cells; i++)
    {
        ...
        for (j = 0; j < 4; j++)
        {
            ...
            {
                fprintf(fout,
                        "        <value cell_index=\"%d\" local_entity=\"%d\" "
                        "value=\"%d\" />\n",
                        i,
                        j,
                        conn[indices[i * 4 + j]]);
                       ...
                       char *boundaries[])
{
    ...
    // Check that number of facet markers equals the number passed boundary
    // names
    ...
    for (i = 0; i < out->num_cells; i++)
    {
        ...
        /*
           // Formula for the determinant of tetrahedron from
              http://mathworld.wolfram.com
           const double det_j = (ax*(by*cz + dy*bz + cy*dz - cy*bz - by*dz -
              dy*cz) -
    ...
    fprintf(fout, "%d\n", out->boundary->numFaces());

    for (auto triangle : out->boundary->faces())
    {
                           ...
                           char *boundaries[])
{
    ...
    for (i = 0; i < out->num_cells; i++)
    {
        ...
        if (out->higher_order)
        {
            ...
            fprintf(fout,
                    "%6d   ElmT10n3D  %3d  %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d \n",
                    i + 1,
                    ss->mat,
                    ss->na + 1,
                    ss->nb + 1,
...
void GemMesh_writeMcsf(GemMesh *out, char *filename)
{
    ...
    for (i = 0; i < out->num_cells; i++)
    {
        ss = out->ss + i;
        fprintf(fout,
                "%10d  0  %5d  %5d %5d %5d %5d  %10d %10d %10d %10d \n",
                i,
                ss->id,
                ss->fa,
                ss->fb,
...
//  // In Tetgen, it could be radmonly 1 or 2 (or even other integers if more
// materials are present)
//  fflush(stdout);
//  exterior_cell_type = -1;
//  for (i = 0; i < numberoftetrahedra; i++) {
//    cell_type = (int)tetrahedronattributelist[i *
...
//  node_attr[node_index] = cell_type==1 ? exterior_node_type :
// interior_node_type;
//      }
//      else if (exterior_cell_type == 2) {
//  if (cell_type >= 3) cell_type = 1;
//  node_attr[node_index] = cell_type==2 ? exterior_node_type :
// interior_node_type;
//      }
//      else {
...
//  fprintf(fout,"%%------------------------------------------------------------------------------
// \n");
//  fprintf(fout,"%%  Node-ID  Chrt        X-Coordinate        Y-coordinate
//        Z-coordinate    \n");
//  fprintf(fout,"%%---------  ----     ----------------     ----------------
//     ---------------- \n");
//
...
//  fprintf(fout,"%%------------------------------------------------------------------------------------------
// \n");
//  fprintf(fout,"%%  Simp-ID Grp    Mat          Face-Types
//                      Vertex-Numbers \n");
//  fprintf(fout,"%%--------- ---    ---    ---------------------
//  ------------------------------------------- \n");
//
...
//      }
//      else {
//  neightype = (int)tetrahedronattributelist[neighbor *
// numberoftetrahedronattributes];
//
//  if (ActiveSite != NULL){
                         ...
                         int output_flag)
{
    ...
    gem_mesh->ss    =
        (FETK_SS *)malloc(sizeof(FETK_SS) * out->numberoftetrahedra);
    ...
    for (i = 0; i < out->numberoftetrahedra; i++)
    {
        ...
        if (ax * dx + ay * dy + az * dz < 0)
        {
            ...
            k                                                  =
                out->neighborlist[i * 4 + 1];
            out->neighborlist[i * 4 +
                              1]         = out->neighborlist[i * 4 + 2];
    ...
    for (i = 0; i < out->numberoftetrahedra; i++)
    {
        ...
        type =
            (int)out->tetrahedronattributelist[i *
        ...
        for (j = 0; j < out->numberofcorners; j++)
        {
            ...
            {
                neightype =
                    (int)out->tetrahedronattributelist[neighbor *
                                                       out->
                                                       numberoftetrahedronattributes
                    ];
    ...
    for (i = 0; i < out->numberoftetrahedra; i++)
    {
        ...
        if (type != ExType)
        {
            ...
            {
                TetraNode[j] =
                    map_w_i[out->tetrahedronlist[i * out->numberofcorners + j] -
                            1];
            }
    ...
    for (i = 0; i < out->numberoftetrahedra; i++)
    {
        ...
        if (type == ExType)
        {
            ...
            {
                TetraNode[j] =
                    map_w_o[out->tetrahedronlist[i * out->numberofcorners + j] -
                            1];
            }
...
// void tetgenio::write_pdb_mcsf(char *inputfilename, float radius, float
// centerx,
//        float centery, float centerz, char *ActiveSite,
//        int output_flag)
// {
...
//      z = pointlist[i * 3 + 2];
//      dist =
// sqrt((x-centerx)*(x-centerx)+(y-centery)*(y-centery)+(z-centerz)*(z-centerz));
//      if (dist > 0.99*radius)
//  NodeAttr[i] = 4;
//      else {
//  NodeAttr[i] = 3;
...
//  fprintf(fout,"%%------------------------------------------------------------------------------------
// \n");
//    fprintf(fout,"%%  Node-ID        Chrt         X-Coordinate
