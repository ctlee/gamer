                                   ...
                                   char *tetgen_params)
{
    ...
    for (sm_index = 0; sm_index < num_surface_meshes; sm_index++)
    {
        ...
        if ((surfmesh->numVertices() == 0) || (surfmesh->numFaces() == 0))
        {
        ...
        num_vertices += surfmesh->numVertices();
        num_faces    += surfmesh->numFaces();
    }
    ...
    for (sm_index = 0; sm_index < num_surface_meshes; sm_index++)
    {
        ...
        for (auto face : surfmesh->faces())
        {
            ...
            p->vertexlist[0]                  = face.a + in.firstnumber + num_vertices;
            p->vertexlist[1]                  = face.b + in.firstnumber + num_vertices;
            p->vertexlist[2]                  = face.c + in.firstnumber + num_vertices;
            in.facetmarkerlist[j + num_faces] = face.m == -1 ? 0 : face.m;
        }

        // Get region or hole coordinate by using a face
        face0 = surfmesh->face_attr(0);

        // Face normal
        normal = GetCrossProduct(surfmesh, face0.a, face0.b, face0.c);

        // Midpoint of face
        midpoint.x = surfmesh->vertex_attr(face0.a).x / 3;
        midpoint.y = surfmesh->vertex_attr(face0.a).y / 3;
        midpoint.z = surfmesh->vertex_attr(face0.a).z / 3;

        midpoint.x += surfmesh->vertex_attr(face0.b).x / 3;
        midpoint.y += surfmesh->vertex_attr(face0.b).y / 3;
        midpoint.z += surfmesh->vertex_attr(face0.b).z / 3;

        midpoint.x += surfmesh->vertex_attr(face0.c).x / 3;
        midpoint.y += surfmesh->vertex_attr(face0.c).y / 3;
        midpoint.z += surfmesh->vertex_attr(face0.c).z / 3;

        // Weight based on length
        double weight = sqrt(pow(surfmesh->vertex_attr(face0.a).x -     \
                                 surfmesh->vertex_attr(face0.b).x, 2) + \
                             pow(surfmesh->vertex_attr(face0.a).y -     \
                                 surfmesh->vertex_attr(face0.b).y, 2) + \
                             pow(surfmesh->vertex_attr(face0.a).z -     \
                                 surfmesh->vertex_attr(face0.b).z, 2));
        ...
        if (!surfmesh->isHole())
        {
            ...
            if (surfmesh->useVolumeConstraint())
            {
                printf("Volume constraint: %.5f\n", surfmesh->volumeConstraint());
                in.regionlist[num_regions * 5 + 4] = surfmesh->volumeConstraint();
            }
            else
            {
...
GemMesh* GemMesh_fromTetgen(tetgenio& tetio)
{
    ...
    for (i = 0; i < tetio.numberoftrifaces; i++)
    {
        ...
        for (j = 0; j < 2; j++)
        {
            ...
            if (on_boundary[j])
            {
                auto face = boundary->face_attr(boundary_face);
                face.a = triface->points[0];
                face.b = triface->points[1];
                face.c = triface->points[2];

                if (tetio.trifacemarkerlist != NULL)
                {
                    face.m = tetio.trifacemarkerlist[i];
                }
                else
                {
                    face.m = 1;
                ...
                boundary->face_attr(boundary_face, face);
