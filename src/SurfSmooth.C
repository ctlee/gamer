...
void GenerateHistogram(SurfaceMeshOld *surfmesh)
{
    ...
    for (m = 0; m < 18; m++)
    {
        printf("%f  ", 100.0 * (float)histogram[m] / ((float)surfmesh->numFaces() * 3.0));
    }
    printf("\n\n");
}
                         ...
                         bool         preserve_ridges)
{
    ...
    getMinMaxAngles(&min_angle, &max_angle, &num_small,
    ...
    while (!smoothed && i < max_iter)
    {
        ...
        for (n = 0; n < num_vertices; n++)
        {
            ...
            MoveVerticesSurfaceOnly(this, n);
            EdgeFlipping(this, n, preserve_ridges);
        }
...
void SurfaceMeshOld::normalSmooth()
{
    ...
    getMinMaxAngles(&min_angle, &max_angle,
                         ...
                         float max_normal_angle)
{
    ...
    vertex_index = (int *)malloc(sizeof(int) * num_vertices);
    face_index   = (int *)malloc(sizeof(int) * num_faces);
    ...
    if (denseness_weight > 0.0)
    {
        ...
        {
            avglen = average_len / (float)(num_faces);
        }
    }
        ...
        if (delete_flag)
        {
            ...
            if (denseness_weight > 0.0)
            {
                while (first_ngr != NULL)
                {
                    ...
                    nx = sqrt((x - vertex[a].x) * (x - vertex[a].x) +
                              (y - vertex[a].y) * (y - vertex[a].y) +
                              (z - vertex[a].z) * (z - vertex[a].z));
                    ny = sqrt((x - vertex[b].x) * (x - vertex[b].x) +
                              (y - vertex[b].y) * (y - vertex[b].y) +
                              (z - vertex[b].z) * (z - vertex[b].z));
            ...
            if (delete_vertex)
            {
                ...
                PolygonSubdivision(this, neighbor_list[n], face_available_list,
                ...
                for (m = 0; m < neighbor_number; m++)
                {
                    ...
                    while (first_ngr != NULL)
                    {
                        ...
                        c        = second_ngr->b;
                        pos_vect = GetPositionSurfaceOnly(x, y, z, b, a, c, this);
                        angle    = GetDotProduct(this, b, a, c);
                        angle   += 1.0;
                        nx      += angle * pos_vect.x;
                        ny      += angle * pos_vect.y;
                        nz      += angle * pos_vect.z;
                    ...
                    if (weight > 0)
                    {
                        ...
                        eigen_vect = GetEigenVector(this, num, &eigen_value, &max_angle);
                        ...
                        else
                        {
                            ...
                            w3 = (nx * eigen_vect.x3 + ny * eigen_vect.y3 + nz * eigen_vect.z3) /
                                 (1.0 + eigen_value.z);
                            vertex[num].x = w1 * eigen_vect.x1 + w2 * eigen_vect.x2 +
                                            w3 * eigen_vect.x3 + x;
                            vertex[num].y = w1 * eigen_vect.y1 + w2 * eigen_vect.y2 +
                                            w3 * eigen_vect.y3 + y;
                            vertex[num].z = w1 * eigen_vect.z1 + w2 * eigen_vect.z2 +
                                            w3 * eigen_vect.z3 + z;
                        }
                    }
                }
    ...
    for (n = 0; n < num_vertices; n++)
    {
            ...
            (vertex[n].z != -99999))
        {
            if (start_index != n)
            {
                vertex[start_index].x      = vertex[n].x;
                vertex[start_index].y      = vertex[n].y;
                vertex[start_index].z      = vertex[n].z;
                vertex[start_index].m      = vertex[n].m;
                vertex[start_index].sel    = vertex[n].sel;
    ...
    for (n = 0; n < num_faces; n++)
    {
        ...
        b           = face[n].b;
        c           = face[n].c;
        face_marker = face[n].m;

        if ((a >= 0) && (b >= 0) && (c >= 0) &&
            (vertex_index[a] >= 0) && (vertex_index[b] >= 0) && (vertex_index[c] >= 0))
        {
            face[start_index].a = vertex_index[a];
            face[start_index].b = vertex_index[b];
            face[start_index].c = vertex_index[c];
            face[start_index].m = face_marker;
                        ...
                        int *face_available_index, int face_marker)
{
    ...
    if (number == 3)
    {
        ...
        auto face = surfmesh->face_attr(face_index);
        face.a = a;
        face.b = b;
        face.c = c;
        face.m = face_marker;
        surfmesh->face_attr(face_index, face);
...
void MoveVerticesSurfaceOnly(SurfaceMeshOld *surfmesh, int n)
{
    ...
    x = surfmesh->vertex_attr(n).x;
    y = surfmesh->vertex_attr(n).y;
    z = surfmesh->vertex_attr(n).z;
    ...
    if (weight > 0)
    {
            ...
            ((eigen_vect.x3 == 0) && (eigen_vect.y3 == 0) && (eigen_vect.z3 == 0)))
        {
            ...
            //	     nx, ny, nz);
            auto vertex = surfmesh->vertex_attr(n);
            vertex.x    = nx;
            vertex.y    = ny;
            vertex.z    = nz;
            surfmesh->vertex_attr(n, vertex);
        }
        else
        {
            nx -= x;
...
FLTVECT GetPositionSurfaceOnly(float x, float y, float z, int a, int b, int c, SurfaceMeshOld *surfmesh)
{
    ...
    bx = surfmesh->vertex_attr(b).x;
    by = surfmesh->vertex_attr(b).y;
    bz = surfmesh->vertex_attr(b).z;
    cx = surfmesh->vertex_attr(c).x;
                                  ...
                                  int max_min_angle, int min_max_angle)
{
    ...
    for (n = 0; n < num_faces; n++)
    {
        ...
        angle = GetAngleSurfaceOnly(this, a, b, c);
        ...
        angle = GetAngleSurfaceOnly(this, b, a, c);
        ...
        angle = GetAngleSurfaceOnly(this, c, a, b);
...
FLTVECT GetNormals(SurfaceMeshOld *surfmesh, int n)
{
    ...
    while (first_ngr != NULL)
    {
        ...
        ax     = surfmesh->vertex_attr(a).x - x;
        ay     = surfmesh->vertex_attr(a).y - y;
        az     = surfmesh->vertex_attr(a).z - z;
        ...
        bx     = surfmesh->vertex_attr(b).x - x;
        by     = surfmesh->vertex_attr(b).y - y;
        bz     = surfmesh->vertex_attr(b).z - z;
...
float GetDotProduct(SurfaceMeshOld *surfmesh, int a, int b, int c)
{
    ...
    bx     = surfmesh->vertex_attr(b).x - surfmesh->vertex_attr(a).x;
    by     = surfmesh->vertex_attr(b).y - surfmesh->vertex_attr(a).y;
    bz     = surfmesh->vertex_attr(b).z - surfmesh->vertex_attr(a).z;
    length = sqrt(bx * bx + by * by + bz * bz);
    ...
    cx     = surfmesh->vertex_attr(c).x - surfmesh->vertex_attr(a).x;
    cy     = surfmesh->vertex_attr(c).y - surfmesh->vertex_attr(a).y;
    cz     = surfmesh->vertex_attr(c).z - surfmesh->vertex_attr(a).z;
    ...
    while (first_ngr != NULL)
    {
        ...
        if (number <= 3)
        {
            if (number > 0)
            {
                ...
                auto vertex = surfmesh->vertex_attr(n);
                vertex.x    = ax / (float)number;
                vertex.y    = ay / (float)number;
                vertex.z    = az / (float)number;
                surfmesh->vertex_attr(n, vertex);
            }
            return;
        }
        ...
        if (flip_flag)
        {
            ...
            if (flip_check)
            {
                ...
                /* Update face info */
                {
                    auto face = surfmesh->face_attr(f1);
                    face.a = n;
                    face.b = a;
                    face.c = c;
                    surfmesh->face_attr(f1, face);
                }
                {
                    auto face = surfmesh->face_attr(f2);
                    face.a = b;
                    face.b = c; // Switch a and c here to make the face
                    face.c = a; // normal point outward
                    surfmesh->face_attr(f2, face);
                }
                ...
                else
                {
                    ...
                    printf("(%f, %f, %f)\n",
                           surfmesh->vertex_attr(n).x,
                           surfmesh->vertex_attr(n).y,
                           surfmesh->vertex_attr(n).z);
                }
...
void NormalSmooth(SurfaceMeshOld *surfmesh, int n)
{
    ...
    while (first_ngr != NULL)
    {
        ...
        if (length > 0)
        {
            ...
            sv     = Rotate(surfmesh->vertex_attr(n).x - cx, surfmesh->vertex_attr(n).y - cy, surfmesh->vertex_attr(n).z - cz, theta, phi, alpha);
            pos_x += sv.x + cx;
            pos_y += sv.y + cy;
            pos_z += sv.z + cz;
    ...
    {
        auto vertex = surfmesh->vertex_attr(n);
        vertex.x    = pos_x / (float)number;
        vertex.y    = pos_y / (float)number;
        vertex.z    = pos_z / (float)number;
        surfmesh->vertex_attr(n, vertex);
    }
}
