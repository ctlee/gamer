...
#
#   The macro also checks for NumPy installation
#
...
AU_ALIAS([AC_PYTHON_NUMPY], [AX_PYTHON_NUMPY])
...
$ac_numpy_result])
	...
	#
	AC_MSG_CHECKING([for NumPy include path])
	if test -z "$NUMPY_CPPFLAGS"; then
		numpy_path=`$PYTHON -c "import numpy; \
			print (numpy.get_include ());"`
		if test -n "${numpy_path}"; then
			numpy_path="-I$numpy_path"
		fi
		NUMPY_CPPFLAGS=$numpy_path
	fi
	...
	# NOTE: Hake
	# NOTE: No need for $PYTHON_EXTRA_LDFLAGS $PYTHON_EXTRA_LIBS as we 
	# NOTE: are not embedding Python
	#LIBS="$ac_save_LIBS $PYTHON_LDFLAGS $PYTHON_EXTRA_LDFLAGS $PYTHON_EXTRA_LIBS"
	LIBS="$ac_save_LIBS $PYTHON_LDFLAGS"
	CPPFLAGS="$ac_save_CPPFLAGS $PYTHON_CPPFLAGS $NUMPY_CPPFLAGS"
	AC_LANG_PUSH([C])
	AC_LINK_IFELSE([
		AC_LANG_PROGRAM([[
#include <Python.h> 
...
import_array();
		...
		],[pythonenvexists=yes],[pythonenvexists=no])
	AC_LANG_POP([C])
	# turn back to default flags
	CPPFLAGS="$ac_save_CPPFLAGS"
	LIBS="$ac_save_LIBS"

	AC_MSG_RESULT([$pythonenvexists])

        if test ! "x$pythonenvexists" = "xyes"; then
	   AC_MSG_FAILURE([
  ...
  ============================================================================
   ...
   You might also need to install the numpy packages.
  ============================================================================
	   ])
	  PYTHON_VERSION=""
	fi
