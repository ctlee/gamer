#!BPY
"""
Name: 'GAMer mesh improvments (pyubic)'
Blender: 249b
Group: 'Mesh'
...
class GamerUI(uiadaptor):
    def __init__(self, *args, **kwargs):
        ...
        self.mesh_params = {}
        self.mesh_menu_items = ["PDB (Surface)", "PDB (Gauss)", "Lattice", "Sphere"]
    ...
    def save_to_registry(self):
        ...
        def to_registry(registry_dict, guiparams):
            for key, value in guiparams.iteritems():
    ...
    def initWidget(self,id=None):
        ...
        self.mparams["boundary_menu"] = self._addElemt(\
            name="", action=self._update_boundary_menu,\
        ...
        for operation in ["coarse_dense", "coarse_flat", "smooth"]:
            ...
            for key, elemt in self.gparams[operation].iteritems():
                self.labels[operation][key] = self._add_text(elemt["name"], \
                                                             width=elemt["width"])
        for key, elemt in self.mparams.iteritems():
            if key == "name":
        ...
        for key, elemt in self.tetparams.iteritems():
            self.labels[key] = self._add_text(elemt["name"], \
                                              width=elemt["width"])
...
#                           self.buttons["unhide_boundary"]])
    ...
    def import_surface_mesh(self):
        ...
        if file_type in ["PDB (Surface)", "PDB (Gauss)", "Lattice"]:
            self.save_to_registry()
            file_type = self.mesh_menu_items[self.getLong(\
                self.mesh_params["surface_mesh_menu"])]

            suffix = "*.pdb" if "PDB" in file_type else "*.lat"
        
            self.fileDialog("Import", self.import_surface_mesh_from_file_action,\
                            suffix=suffix)
    ...
    def _get_next_marker(self, marker, boundaries=None):
        ...
        markers = [b["marker"] for b in boundaries.values()]
    ...
    def _get_boundary_faces(self, boundary):
        if not "faces" in boundary:
            return []
        all_faces = []
        for faces in boundary["faces"].values():
    ...
    def _set_boundary_faces(self, boundary, faces):
        ...
        if "faces" in boundary:
            for key in boundary["faces"].keys():
                del boundary["faces"][key]
            del boundary["faces"]
            
        boundary["faces"] = {}
        for ind in xrange(num_sub_arrays):
    ...
    def calculate_areas(self):
        ...
        for boundary_name in self.menu_items:
            ...
            if area > 0:
                ...
                print "Area of boundary '%s' %.2e"%(boundary_name, area)
    ...
    def _assign_boundary(self):
        ...
        for bound_name in boundaries.keys():
    ...
    def delete_faces(self):
        ...
        for boundary_name in self.menu_items:
            ...
            if not new_faces:
                print "No faces left for boundary:", boundary_name
    ...
    def triangulate_holes(self):
        ...
        # Sanity checks
        for vertex, neigbors in vertex_vertex.iteritems():
            print vertex, neigbors
            if neigbors[0] not in vertex_vertex:
                print "neigbor not in map:", neigbors[0]
                return
            if neigbors[1] not in vertex_vertex:
                print "neigbor not in map:", neigbors[1]
                return
            if len(neigbors) != 2:
                print "detected a non closed edge loop at vertex: %d"%vertex
                return
        ...
        while vertex_vertex:
            ...
            while 1:
                ...
                if vertex == edge_loop[0]:
                    ...
                    print "Found edge loop:", len(edge_loops), edge_loop
                    break
    ...
    def import_surface_mesh_from_file_action(self, filename):
        ...
        if file_type == "PDB (Surface)":
            ...
            gmesh = gamer.cgamer.SurfaceMesh_readPDB_molsurf(filename)
            gmesh.centralize()
            ...
            gmesh = gamer.cgamer.SurfaceMesh_readPDB_gauss(filename,
                                                           blobbyness, iso_value)
            gmesh.centralize()
            self.gamer_to_host(gmesh, {}, filename.replace(".pdb", ""),
                               switch_layer=False)
